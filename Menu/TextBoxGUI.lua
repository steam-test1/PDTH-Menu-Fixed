...
function TextBoxGui:set_centered()
	self._bg_frame:set_center(self._ws:panel():center())
	self._panel:set_top(self._bg_frame:top())
	self._panel:set_left(self._ws:panel():left() + 30)
end
...
function TextBoxGui:_create_text_box(ws, title, text, content_data, config)
	...
	local font_size = preset and preset.font_size or config and config.font_size or tweak_data.menu.pd2_medium_font_size - 5
	...
	local new_main = ws:panel():panel({
		layer = 999,
		visible = self._visible,
	})
	self._panel = main
	self._fullscreen_panel = new_main
	self._panel_h = self._panel:h()
	self._panel_w = self._panel:w()
	self._bg_frame = new_main:bitmap({
	...
	self._indicator:set_right(tw + 40)
	self._indicator:set_bottom(title_text:bottom() - 2)
	...
	buttons_panel:set_bottom(self._bg_frame:bottom() - 10)
	...
	if has_buttons then
		...
		if button_list then
			for i, button in ipairs(button_list) do
				...
				text:set_left(button_panel:left() + 3)
		...
		buttons_panel:set_left(self._ws:panel():left() + 10)
...
function TextBoxGui:_set_button_selected(index, is_selected)
	...
	if button_panel then
		...
		if is_selected then
			button_text:set_color(PDTHMenu_color_highlight)
			selected:set_shape(button_panel:shape())
			selected:move(2, -1)
		else
			button_text:set_color(PDTHMenu_color_normal)
		end
	end
end
...
function TextBoxGui:set_size(x, y)
	...
	self._fullscreen_panel:set_h(y)
	...
	scroll_panel:set_y(scroll_panel:y() + 1)
	local _,_,w,_ = scroll_panel:child("text"):text_rect()
...
function TextBoxGui:close()
	...
	self._ws:panel():remove(self._fullscreen_panel)
	if alive(self._background) then

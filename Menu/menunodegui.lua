...
function MenuNodeGui:init(node, layer, parameters)
	...
	self._align_line_proportions = node:parameters()._align_line_proportions or 0.75
	...
	self.row_item_color = PDTHMenu_color_normal
	self.row_item_hightlight_color = PDTHMenu_color_highlight
...
function MenuNodeGui:_create_marker(node)
	...
	self._marker_data.gradient = self._marker_data.marker:bitmap({
		...
		y = 0,
		layer = 0,
		color = PDTHMenu_color_highlight
	})
...
function MenuNodeGui:_align_marker(row_item)
	if self.marker_color or row_item.marker_color then
		self._marker_data.gradient:set_color(row_item.item:enabled() and (row_item.marker_color or self.marker_color) or tweak_data.menu.default_disabled_text_color)
	end
...
function MenuNodeGui:_highlight_row_item(row_item, mouse_over)
	if row_item then
		...
		if active_menu then
			if row_item.item:parameters().text_id then
				active_menu.renderer:set_bottom_text(row_item.item:parameters().text_id,row_item.item:parameters().help_id, row_item.item:parameters().localize_help ~= false)
			else
